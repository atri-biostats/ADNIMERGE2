...
# Function to extract failures status ----
## ADNI1 screen fails ----
#' @title Function to extract screen fails in ADNI1 phase
#' @description This function is used to extract screen fails in ADNI1.
#' @param registry_dd Data frame of `REGISTRY` eCRF
#' @param phase ADNI study phase, Default: 'ADNI1'
#' @return A data frame that contains `RID`, `ORIGPROT`, `COLPROT`, `SCREENFAIL_DATE` and `SCREENFAIL` variables.
#' @examples
#' \dontrun{
#' extract_adni1_screen_fails(registry_dd = ADNIMERGE2::REGSITRY)
#' }
#' @seealso [extract_adni34_screen_fails()] [extract_adni_screen_fails()]
#' @rdname extract_adni1_screen_fails
#' @export
extract_adni1_screen_fails <- function(registry_dd, phase = "ADNI1") {
  rlang::arg_match0(arg = phase, values = c("ADNI1"))
  check_colnames(
    dd = registry_dd,
    col_names = c("RID", "ORIGPROT", "COLPROT", "VISCODE", "EXAMDATE"),
    strict = TRUE
  )
  RID <- COLPROT <- ORIGPROT <- EXAMDATE <- VISCODE <- NULL
  SCREENFAIL <- SCREENFAIL_DATE <- participant_type <- NULL
  output_dd <- registry_dd %>%
    filter(COLPROT %in% phase) %>%
    mutate(across(c(COLPROT, ORIGPROT), as.character)) %>%
    # Adding participant types (i.e. study track)
    mutate(participant_type = adni_study_track(
      cur_study_phase = COLPROT,
      orig_study_phase = phase
    )) %>%
    verify(participant_type == "New") %>%
    # Checking for multiple screens with different outcomes
    filter(VISCODE %in% c("f", "sc")) %>%
    assert(is_uniq, RID) %>%
    filter(VISCODE %in% "f") %>%
    mutate(
      SCREENFAIL = "Yes",
    ...
    select(RID, ORIGPROT, COLPROT, SCREENFAIL_DATE = EXAMDATE, SCREENFAIL)
  return(output_dd)
}

## ADNI3 and ADNI4 screen fails -----
#' @title Function to extract screen fails in ADNI3 and ADNI4 phases
#' @description This function is used to extract screen fails in ADNI3 and ADNI4 phases.
#' @param studysum_dd Data frame of `STUDYSUM` eCRF
#' @param phase ADNI study phase, Default: 'NULL' that includes both ADNI3 and ADNI4 phases.
#' @return A data frame that contains `RID`, `ORIGPROT`, `COLPROT`, `SCREENFAIL_DATE` and `SCREENFAIL` variables.
#' @examples
#' \dontrun{
#' extract_adni34_screen_fails(studysum_dd = ADNIMERGE2::STUDYSUM, phase = NULL)
#' extract_adni34_screen_fails(studysum_dd = ADNIMERGE2::STUDYSUM, phase = c("ADNI3", "ADNI4"))
#' }
#' @seealso [extract_adni1_screen_fails()] [extract_adni_screen_fails()]
#' @rdname extract_adni34_screen_fails
#' @export
extract_adni34_screen_fails <- function(studysum_dd, phase = NULL) {
  RID <- COLPROT <- ORIGPROT <- SDSTATUS <- SDDATE <- NULL
  SCREENFAIL <- SCREENFAIL_DATE <- VISCODE <- participant_type <- NULL
  if (is.null(phase)) phase <- c("ADNI3", "ADNI4")
  rlang::arg_match(arg = phase, values = c("ADNI3", "ADNI4"), multiple = TRUE)
  check_colnames(
    dd = studysum_dd,
    col_names = c("RID", "ORIGPROT", "COLPROT", "SDSTATUS", "SDDATE", "VISCODE"),
    strict = TRUE
  )

  # `SDSTATUS` must not contains any numeric value
  detect_numeric_value(
    input_value = studysum_dd$SDSTATUS,
    num_type = "any",
    stop_message = TRUE
  )

  output_dd <- studysum_dd %>%
    mutate(across(c(COLPROT, ORIGPROT), as.character)) %>%
    # Adding participant types (i.e. study track)
    mutate(participant_type = adni_study_track(
      cur_study_phase = COLPROT,
    ...
    filter(participant_type == "New") %>%
    verify(COLPROT == ORIGPROT) %>%
    filter(VISCODE %in% c("sc", "4_disp")) %>%
    assert(is_uniq, RID) %>%
    filter(SDSTATUS == "Never enrolled") %>%
    filter(ORIGPROT %in% phase) %>%
    mutate(
      SCREENFAIL = "Yes",
      ORIGPROT = as.character(ORIGPROT),
      COLPROT = as.character(COLPROT)
    ) %>%
    select(RID, ORIGPROT, COLPROT, SCREENFAIL_DATE = SDDATE, SCREENFAIL)

  return(output_dd)
}

## ADNIG) and ADNI2 screen fails ----
### ??? Required to identify the corresponding data source (eCRF)

## Overall screen fails ----
#' @title Function to extract screen fails in ADNI study (Only limited to ADNI1, ADNI3 and ADNI4 phases)
#' @description This function is used to extract screen fails in for limited ADNI study phases.
#' @param registry_dd Data frame of `REGISTRY` eCRF, Default: 'NULL', and required if `phase` argument is either `Overall` or `ADNI1`
#' @param studysum_dd Data frame of `STUDYSUM` eCRF, Default: 'NULL', and required if `phase` argument is either `Overall`, `ADNI3` or `ADNI4`
#' @param phase ADNI study phase, Default: 'Overall' that includes `ADNI1`, `ADNI3` and `ADNI4` phases.
#' @return A data frame that contains `RID`, `ORIGPROT`, `COLPROT`, `SCREENFAIL_DATE` and `SCREENFAIL` variables.
#' @examples
#' \dontrun{
#' extract_adni_screen_fails(
#'   studysum_dd = ADNIMERGE2::STUDYSUM,
#'   registry_dd = ADNIMERGE::REGISTRY,
#'   phase = "Overall"
#' )
#' extract_adni_screen_fails(
#'   registry_dd = NULL,
...
#' )
#' }
#' @seealso [extract_adni1_screen_fails()] [extract_adni34_screen_fails()]
#' @rdname extract_adni_screen_fails
#' @export
#' @importFrom stats na.omit
extract_adni_screen_fails <- function(registry_dd = NULL, studysum_dd = NULL, phase = "Overall") {
  RID <- COLPROT <- ORIGPROT <- SCREENFAIL_DATE <- SCREENFAIL <- NULL
  # rlang::arg_match(arg = phase, values = c("Overall", adni_phase()), multiple = TRUE)
  rlang::arg_match(
    arg = phase,
    values = c("Overall", "ADNI1", "ADNI3", "ADNI4"),
    multiple = TRUE
  )
  data_frame_null <- tibble(
    RID = NA_real_, ORIGPROT = NA_character_, COLPROT = NA_character_,
    SCREENFAIL_DATE = NA_character_, SCREENFAIL = NA_character_
  ) %>%
    mutate(SCREENFAIL_DATE = as.Date(SCREENFAIL_DATE)) %>%
    na.omit()

  if (any(phase %in% c("ADNI1", "Overall"))) {
    if (is.null(registry_dd)) stop("`registry_dd` must be provided")
    output_dd_registry <- extract_adni1_screen_fails(
      registry_dd = registry_dd,
      phase = "ADNI1"
    ) %>%
      select(RID, ORIGPROT, COLPROT, SCREENFAIL_DATE, SCREENFAIL) %>%
      mutate()
  } else {
    output_dd_registry <- data_frame_null
  }

  if (any(phase %in% c("ADNI3", "ADNI4", "Overall"))) {
    if (is.null(studysum_dd)) stop("`studysum_dd` must be provided")
    output_dd_studysum <- extract_adni34_screen_fails(
      studysum_dd = studysum_dd,
      phase = NULL
    ) %>%
      select(RID, ORIGPROT, COLPROT, SCREENFAIL_DATE, SCREENFAIL)
  } else {
    output_dd_studysum <- data_frame_null
  }

  # ADNI1 screen fails
  output_dd <- output_dd_registry %>%
    # ADNI3 and ADNI4 screen fails flags
    bind_rows(output_dd_studysum) %>%
    {
      if (all(phase %in% "Overall")) {
        (.)
      } else {
