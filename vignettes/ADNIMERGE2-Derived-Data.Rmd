...
MAPPING_ID <- tibble(PTID = unique(c(ROSTER$PTID, REGISTRY$PTID, PTDEMOG$PTID))) %>%
  mutate(
    RID = as.numeric(str_remove_all(string = PTID, pattern = "^[0-9]{3}\\_S\\_")),
...
```{r include-origprot-colprot, class.source = NULL, echo = FALSE}
...
[`DM`](../reference/DM.html) dataset contains one records per participant when participant were enrolled or screened in the ADNI study for the first time (i.e. as new-enrollee). The [`DM`](../reference/DM.html) dataset will contains the following characteristics:
...
[`AE`](../reference/AE.html) dataset contains one records per adverse event per participant. The [`AE`](../reference/AE.html) dataset will includes the following characteristics for participant that have at least one adverse events experience during the study.
...
AE_ADNI34 <- ADVERSE %>%
  ...
  filter((n() > 1 & row_number() == n()) | (n() == 1 & row_number() == 1)) %>%
  ungroup() %>%
...
[`QS`](../reference/QS.html) dataset contains one record per parameter finding (i.e. total score) per visit per participant. The [`QS`](../reference/QS.html) dataset will contains the following characteristics:
...
The following total and sub-scores will be included the [`QS`](../reference/QS.html) dataset. 
...
QS <- bind_rows(
  ...
  assert_rows(num_row_NAs, within_bounds(0, 2.1), VISCODE) %>%
...
[`RS`](../reference/RS.html) dataset will contains one record per instrument status per visit per participant. The [`RS`](../reference/RS.html) dataset will contains the following characteristics:
...
The [`RS`](../reference/RS.html) domain will contains the diagnostics status of participant per the scheduled study visit. The clinical diagnostics status (i.e. either Cognitive Normal (CN), Mild Cognitive Impairment (MCI) or Dementia/Alzheimer's (DEM/AD)) of a participant was determined by a clinician judgment.
...
RS_BASELINE_FLG <- RS %>%
  ...
  filter(VISITNUM <= 1 & RSDTC <= RFSTDTC + 90) %>%
...
[`NV`](../reference/NV.html) domain contains one record of physiological and morphological finding related to the nervous system (including brain) per visit per participant. The [`NV`](../reference/NV.html) dataset will contains the following characteristics:
...
* **Amyloid Status: ** The amyloid status (i.e. elevated vs non-elevated) of a participant was determined by a clinician judgment after consensus session review if required. At this moment, the amyloid status for the ADNI4 participants is only included. 
...
C2N_PLASMA <- C2N_PRECIVITYAD2_PLASMA %>%
  ...
  mutate(BSTESTCD = toupper(str_remove_all(
    string = BSTESTCD,
    pattern = "\\_C2N|\\_"
  ))) %>%
  mutate(
    BSTESTCD = case_when(
      BSTESTCD %in% "PT217NPT217" ~ "PT217NPT",
      TRUE ~ BSTESTCD
    ),
    BSNAM = "C2N",
    BSSPEC = "PLASMA"
...
[`VS`](../reference/VS.html) domain contains measurement finding of blood pressure, heart beat, respiratory rate,  temperature, weight and height per visit per participant. The [`VS`](../reference/VS.html) dataset will contains the following characteristics:
