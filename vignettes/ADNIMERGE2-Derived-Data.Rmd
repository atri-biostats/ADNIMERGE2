...
* *ORIGPROT* variable is included in `DM` data to identify the first study protocol/phase of subjects' enrollment in the ADNI study. 
...
* *VISITNUM* variable is included only for creating study epoch across the study phases and mainly used for data merging and sorting purpose. The *VISITNUM* variable should not be used in analysis related to these derived dataset. Please refer to `Data Preparation` section below to learn more about how the study visit number *VISITNUM* is created.
...
[`DM`](../reference/DM.html) data contains one record per subject when subjects were enrolled or screened in the ADNI study for the first time (i.e. as new-enrollee).
...
SC_RURALITY <- RURALITY %>%
  ...
  # Convert into numeric scale
  mutate(
    RUCA_2010 = as.numeric(RUCA_2010),
    RUCC_2023 = case_when(str_detect(RUCC_2023, "99 -") ~ 99,
      .default = as.numeric(RUCC_2023)
    )
  ) %>%
  # Create RURALITY var - combining binary and coded RUCA value
  mutate(
    RUCAGR = case_when(
      RUCA_2010 >= 1 & RUCA_2010 <= 3 ~ "Urban",
      RUCA_2010 >= 4 & RUCA_2010 <= 10 ~ "Rural",
      .default = RUCA
    )
  ) %>%
  mutate(across(all_of(c(rurality_cols, "RUCAGR", "SCDTC")), as.character)) %>%
  pivot_longer(
    cols = all_of(c(rurality_cols, "RUCAGR")),
    names_to = "SCTESTCD",
    values_to = "SCORRES"
  ) %>%
...
# Adjust for derived RUCAGR vars
SC <- SC %>%
  mutate(
    SCSTRESC = ifelse(SCTESTCD == "RUCAGR", SCORRES, NA_character_),
    SCORRES = ifelse(SCTESTCD == "RUCAGR", NA_character_, SCORRES)
  )
```
...
# Add RUCAGR variable description
SC_TESTCD_LIST <- bind_rows(
  SC_TESTCD_LIST,
  tibble::tibble(
    SCTESTCD = "RUCAGR",
    SCTEST = "Derived RUCA Designation Category"
  )
)
```
...
VS <- VS %>%
  ...
  mutate(across(
    all_of(c("VSORRES", "VSORRESU", "VSLOC")),
    ~ case_when(
      .x == "NA" ~ NA_character_,
      TRUE ~ as.character(.x)
    )
  ...
  # Set standard unit of measurements
  mutate(
    VSSTRESU = case_when(
      VSORRESU %in% c("F", "C") ~ "C",
