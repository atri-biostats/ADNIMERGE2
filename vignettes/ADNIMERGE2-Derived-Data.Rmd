...
EPOCH_LIST <- VISITS %>%
  ...
  select(Phase, VISCODE, PHASE_EPOCH, EPOCH)
```
...
AE_ADNI34 <- ADVERSE %>%
  ...
  rename(
    "AESER" = AESERIOUS, "AEOUT" = AEOUTCOME, # "AEPATT" =
...
# Required checking for missing AENUMBER
...
ECOG_SCORE_DD <- ECOGPT %>%
  select(all_of(qs_com_cols), "ECOGPTTOTAL" = EcogPtTotal) %>%
  bind_rows(ECOGSP %>%
    select(all_of(qs_com_cols), "ECOGSPTOTAL" = EcogSPTotal)) %>%
  # ECOG12-item score is not available yet
  pivot_longer(
    cols = c(ECOGPTTOTAL, ECOGSPTOTAL),
    names_to = "QSTESTCD",
    values_to = "QSSTRESN"
  ) %>%
  mutate(
...
QS <- bind_rows(
  ...
  mutate(COLPROT = factor(COLPROT, levels = adni_phase())) %>%
  group_by(RID) %>%
  arrange(QSTESTCD, COLPROT, VISCODE) %>%
