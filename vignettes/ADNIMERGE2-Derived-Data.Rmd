...
SC_RURALITY <- RURALITY %>%
  ...
  mutate(across(all_of(c(rurality_cols, "RURALITY", "SCDTC")), as.character)) %>%
  pivot_longer(
    cols = all_of(c(rurality_cols, "RURALITY")),
    names_to = "SCTESTCD",
    values_to = "SCORRES"
  ) %>%
...
# Adjust for derived RURALITY vars
SC <- SC %>%
  mutate(
    SCSTRESC = ifelse(SCTESTCD == "RURALITY", SCORRES, NA_character_),
    SCORRES = ifelse(SCTESTCD == "RURALITY", NA_character_, SCORRES)
  )
```
...
# Add rurality variable description
SC_TESTCD_LIST <- bind_rows(
  SC_TESTCD_LIST,
  tibble::tibble(
    SCTESTCD = "RURALITY",
    SCTEST = "Derived rurality binary category"
  )
)
```
