...
adsl_dic <- bind_rows(
  ...
  c(
    variable = "RUCAGR1", type = "text", length = "10", label = "RUCA Designation", code_id = "RUCAGR1", origin = "Derived",
    derivation = "SC.SCSTRESC where SC.SCTESTCD='RUCAGR' and SCBLFL = 'Y'."
  ),
  c(
...
# Rurality binary category ---
RUCAGR1_CODEDLIST <- tibble(
  code_id = "RUCAGR1", name = "RUCAGR1", type = "code_decode",
  tibble(
    code = c("Urban", "Rural"),
    decode = c("Urban", "Rural")
  )
)
...
QSTESTCD_LIST <- c(
  ...
  "LIMMTOTL", "MMSCORE", "MOCA", "NPIQTOTL", "NPITOTAL", "RAVLTFG", "RAVLTFGP", 
  "RAVLTIMM","RAVLTLRN", "TRABSCOR"
)
if (params$INCLUDE_PACC == TRUE){
  QSTESTCD_LIST <- c(QSTESTCD_LIST, "MPACCDIGIT", "MPACCTRAILSB")
}
QSTESTCD_LIST <- sort(QSTESTCD_LIST)
