...
This document describes creating a metadata specification using [PHARMAVERSE]() workflow. The matadata-specs can be used to generate analysis ready dataset (i.e., ADaM). The matadata-specs required standardized derived dataset, such as SDTM, as input. For illustration purpose, a metadata-specs for following selected analysis ready dataset will be created.
...
adsl_dic <- bind_rows(
  ...
  c(variable = "ORIGPROT", type = "text", length = "8", label = "Original Study Protocol", origin = "Predecessor", derivation = "DM.ORIGPROT"),
) %>%
  as_tibble() %>%
  mutate(
    dataset = "ADSL",
...
adae_dic <- bind_rows(
  ...
  c(variable = "AERELFLRBPR", type = "text", length = "40", label = "Related to PET/MRI Imaging Procedure", code_id = "REL", origin = "Predecessor", derivation = "AE.AERELFLRBPR"),
...
ds_spec <- data_spec %>%
  ...
  distinct() %>%
  assert_rows(num_row_NAs, within_bounds(0, 0.5), everything()) %>%
...
ds_vars <- data_spec %>%
  ...
  assert_rows(num_row_NAs, within_bounds(0, 0.5), !key_seq) %>%
...
derivations_labels <- as.list(c(
  derivation_id = "ID of Derivation",
  derivation = "Derivation"
))
derivations <- data_spec %>%
  select(all_of(names(derivations_labels))) %>%
  assert_rows(num_row_NAs, within_bounds(0, 0.05), derivation_id)
```
...
```{r check-common-variables, echo = FALSE}
