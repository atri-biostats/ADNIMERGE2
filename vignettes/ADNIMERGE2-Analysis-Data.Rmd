...
datatable <- function(data, paging = FALSE, searching = TRUE, bInfo = FALSE, ...) {
  DT::datatable(
    data = data, ...,
    options = list(
      paging = paging, searching = searching, bInfo = bInfo,
      ...
    )
  )
}
...
bind_rows(
  ...
  datatable(searching = FALSE, bInfo = FALSE)
```
...
# Add derived baseline RURALITY - only collected in ADNI4 phase
ADSL <- ADSL %>%
  derive_vars_merged(
    dataset = .,
    dataset_add = SC,
    filter_add = SCTESTCD == "RURALITY" & SCBLFL == "Y",
    new_vars = exprs(RURALITY = SCSTRESC),
    order = exprs(VISITNUM),
    mode = "last",
    by_vars = exprs(STUDYID, USUBJID),
    check_type = "error",
