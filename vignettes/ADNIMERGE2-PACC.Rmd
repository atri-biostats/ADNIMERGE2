...
The next step is to calculate component specific baseline summaries that will be used to normalize component score. This baseline summary is based on user-defined standardization population. Here, we define the standardization population as follows:
...
The composite PACC score is the sum of the standardized component scores adjusted by the number of components. The following two conditions are required to be checked before computing the PACC score.
...
The `ADNIMERGE2::compute_pacc_score` function can be used to compute the two modified composite PACC scores in ADNI study. The next r chunk shows how to calculate the PACC composite score based on a wide format input component score, a standardization baseline summary, and with a logarithm transformation of `Trials B` score inside in the scoring function.
...
The following r chunks show how to get a baseline PACC composite scores based on the `PACC` data. Similar result can be obtained using [`ADSL`](../reference/ADSL.html) or [`ADQS`](../reference/ADQS.html) datasets. For more information regarding how to get the baseline PACC scores from those derived dataset, please see [ADNI-Enrollment](../articles/ADNI-Enrollment.html) and [ADNIMERGE2-Analysis-Data](../articles/ADNIMERGE2-Analysis-Data.html) articles.
