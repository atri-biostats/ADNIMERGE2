...
b. **mPACCtrailsB: ** Using a logarithm scale of `Trials B` score as a proxy for DSST, and the score may available across all ADNI study phases.
...
pacc_data_dic <- bind_rows(
  ...
  c(
    ...
    LABEL = "Logarithm Trials B Standardized Score",
    TYPE = "numeric",
    TEXT = paste0(
...
* `pacc_neurobat`: A dataset contains Logical Memory - Delayed Recall (`LDELTOTL`), Digit Symbol Substitution (`DIGITSCR`), and Trials B (`TRABSCOR`) raw scores from Neuropsychological Battery (`NEUROBAT`) table. The `NEUROBAT()` data is also downloaded from the data-sharing platform.
...
* `Trials B` Measurement Scale: To identify whether the score is in a logarithm scale. For example, the `Trials B` score is not in a logarithm scale. We need either to map the raw score values to logarithm scale prior to using a scoring function or to specify an argument that allows us to perform a logarithm transformation inside a scoring function.

* Standardized Components Score: Each component scores are normalized.

* Standardized Component Directional Effect: To verify the same directional effect across standardized component score, and reoriented standardized component scores as needed since the composite score is the sum of these standardized scores. In ADNI study, `ADAS-cog Q4` and `Trials B` scores have opposite directional effect compared to the remaining PACC component scores. Higher values of those two components indicates worse condition. Thus, the directional effect of those two standardized components are required to be re-oriented as the remaining components. As a result, higher values of the composite score would indicate better condition/higher performance in cognition.
...
The next r chunk shows how to calculate the PACC composite score based on a wide format input component score, a standardization baseline summary, and with a logarithm transformation of `Trials B` score inside in the scoring function.
